...
2. Create a folder named `my-little-guy` inside your vault's `.obsidian/plugins/` folder.
3. Place the three files inside it.
4. In Obsidian, go to **Settings → Community plugins**, reload the plugin list, and enable **My Little Guy**.

### On mobile (iOS/Android), or anywhere you can't easily reach `.obsidian/`

iOS in particular hides the `.obsidian` folder from Files app by default, which makes copying plugin files in manually unreliable. Use **[BRAT](https://github.com/TfTHacker/obsidian42-brat)** instead — it installs plugins over the network, from inside Obsidian itself, so it never touches the filesystem directly:

1. Install **BRAT** from **Settings → Community plugins → Browse** (it's an official, listed plugin — this step works normally on any platform).
2. Open BRAT's settings and tap **"Add Beta Plugin"**.
3. Enter this repository: `glenhiggy-sketch/My-Little-Guy`
